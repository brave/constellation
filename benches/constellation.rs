...
pub fn client(c: &mut Criterion) {
    c.bench_function("client prepare",
        |b| b.iter(|| prepare(black_box(THRESHOLD)).unwrap()));
}

pub fn server(c: &mut Criterion) {
    let messages: Vec<_> = (0..THRESHOLD).map(|_| prepare(THRESHOLD).unwrap()).collect();
    c.bench_function("server aggregate",
        |b| b.iter(|| aggregate(black_box(&messages))));
}
